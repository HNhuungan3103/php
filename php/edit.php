    ...
    if ($new_file["error"] == UPLOAD_ERR_OK) {  // Kiểm tra xem không có lỗi khi tải lên
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($new_file["tmp_name"], $target_file)) {
                // Xóa ảnh cũ nếu có
                $sql = "SELECT image FROM products WHERE id = '$file_id'";
                ...
                $sql = "UPDATE products SET name='$new_name', image='$target_file' WHERE id='$file_id'";
        ...
        $sql = "UPDATE products SET name='$new_name' WHERE id='$file_id'";
        if ($conn->query($sql) === TRUE) {
            echo "Cập nhật thành công!<br>";
        } else {
            echo "Lỗi: " . $conn->error;  // Hiển thị lỗi nếu có
...
if (isset($_GET['delete_id'])) {
    ...
    $sql = "SELECT image FROM products WHERE id = '$delete_id'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $file_path = $row['image'];
    ...
    if ($new_file["error"] == UPLOAD_ERR_OK) {  // Kiểm tra xem không có lỗi khi tải lên
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($new_file["tmp_name"], $target_file)) {
                ...
                $sql = "INSERT INTO products (name, image) VALUES ('$new_name', '$target_file')";
                if ($conn->query($sql) === TRUE) {
...
if (isset($_GET['file_id'])) {
    ...
    $sql = "SELECT * FROM products WHERE id = '$file_id'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();  // Lấy thông tin sản phẩm
        $file_name = $row['name'];  // Tên sản phẩm
        $file_path = $row['image'];  // Đường dẫn ảnh
    } else {
...
$sql = "SELECT * FROM products ORDER BY id DESC LIMIT 4";  // Lấy 4 ảnh mới nhất
$result = $conn->query($sql);
?>
    ...
    <div class="image-container">
        ...
        <div>
            ...
            <div class="image-info">
                Tên sản phẩm: <?php echo htmlspecialchars($row['name']); ?><br>
                <a href="?file_id=<?php echo $row['id']; ?>">Chỉnh sửa</a>
            </div>
